...
var C = {
  ...
  l2s:function(y,m,d){
    y=+y;m=+m;d=+d;
    var day=this.mD(y,m);
    if(y<1900||y>2100||d>day||d<1)return null;
    var off=0;
...
function legalList(year){
  ...
  return [
    ["元旦",fmtYMD(year,1,1)],
    ["春节",ls(year,1,1)],
    ["清明节",t7>0?fmtYMD(year,4,t7):null],
    ["劳动节",fmtYMD(year,5,1)],
    ["端午节",ls(year,5,5)],
    ["中秋节",ls(year,8,15)],
    ["国庆节",fmtYMD(year,10,1)]
  ].filter(x=>x[1]).sort((a,b)=>new Date(a[1])-new Date(b[1]));
}
...
function intlList(year){
  return [
    ...
    ["母亲节",nthWeekday(year,5,0,2)],
    ["父亲节",nthWeekday(year,6,0,3)],
...
function top3(all, today){
  var fut=all.filter(x=>dateDiff(today,x[1])>=0);
  var take=fut.slice(0,3);
  var i=0;
  while(take.length<3&&i<all.length){
    take.push(all[i++]);
  }
...
export default async function(ctx){
  ...
  function dds(arr){return arr.map(x=>dateDiff(today,x[1]));}
