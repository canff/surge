...
export default async function (ctx) {
  ...
  try {
    ...
    // 🛠️ 暴力修复私钥格式：应对 Egern 环境变量框对换行符的“吞噬”
    ...
    const cmds = [
      ...
      // 采集最后执行的命令，兼容bash/zsh
      "tail -1 ~/.bash_history 2>/dev/null | sed 's/^ *//' || tail -1 ~/.zsh_history 2>/dev/null | sed 's/^ *//' || echo '无历史命令'", // 12
    ];
    ...
    d = {
      ...
      rxRate, txRate, netRx, netTx, temp, procs, lastCmd,
    };
  } catch (e) {
  ...
  // ─── Theme ──────────────────────────────────
  const C = {
    bg1: '#0f1218', bg2: '#171b24',
    barBg: '#2a2f3a',
    text: '#f0f4f9', muted: '#a1a8b5', dim: '#6b7280',
    cpu: '#34d399', mem: '#60a5fa', swap: '#a78bfa',
    net: '#f472b6', disk: '#fbbf24', temp: '#f87171',
    // 新增Last Cmd专属高亮色，和整体风格协调，肉眼清晰可见
    cmd: '#f472b6'
  };
        ...
        { type: 'stack', direction: 'column', gap: 6, children: [
          ...
          { type: 'stack', direction: 'column', gap: 2, children: [
            { type: 'stack', direction: 'row', alignItems: 'center', gap: 4, children: [
              ...
              { type: 'spacer' },
              { type: 'text', text: `Load ${d.load.join(' ')}`, font: { size: 10, family: 'Menlo' }, textColor: C.cpu },
            ]},
          ...
          { type: 'stack', direction: 'column', gap: 2, children: [
            { type: 'stack', direction: 'row', alignItems: 'center', gap: 4, children: [
              ...
              { type: 'spacer' },
              { type: 'text', text: `${fmtBytes(d.memUsed)} / ${fmtBytes(d.memTotal)}`, font: { size: 10, family: 'Menlo' }, textColor: C.mem },
            ]},
          ...
          { type: 'stack', direction: 'column', gap: 2, children: [
            { type: 'stack', direction: 'row', alignItems: 'center', gap: 4, children: [
              ...
              { type: 'spacer' },
              { type: 'text', text: `${fmtBytes(d.diskUsed)} / ${fmtBytes(d.diskTotal)}`, font: { size: 10, family: 'Menlo' }, textColor: C.disk },
            ]},
          ...
          { type: 'stack', direction: 'row', alignItems: 'center', gap: 4, children: [
            ...
            { type: 'spacer' },
            { type: 'text', text: `↓${fmtBytes(d.netRx)}`, font: { size: 10, family: 'Menlo' }, textColor: C.net },
            { type: 'text', text: `↑${fmtBytes(d.netTx)}`, font: { size: 10, family: 'Menlo' }, textColor: C.swap },
          ]},
          // Last Cmd 行：字体放大到13号+专属彩色高亮
          { type: 'stack', direction: 'row', alignItems: 'center', gap: 4, children: [
            { type: 'image', src: 'sf-symbol:terminal', color: C.muted, width: 11, height: 11 },
            { type: 'text', text: 'Last Cmd', font: { size: 'caption1', weight: 'semibold' }, textColor: C.text },
            { type: 'spacer' },
            { type: 'text', text: d.lastCmd, font: { size: 13, family: 'Menlo', weight: 'medium' }, textColor: C.cmd, maxLines: 1, minScale: 0.6 },
          ]},
        ]},
        { type: 'spacer' },
        footer,
    ...
    children: [
      ...
      ...(d.swapTotal > 0 ? [
        { type: 'stack', direction: 'row', alignItems: 'center', children: [
          ...
          { type: 'spacer' },
          { type: 'text', text: `${fmtBytes(d.swapUsed)} / ${fmtBytes(d.swapTotal)}`, font: { size: 10, family: 'Menlo' }, textColor: C.swap },
        ]},
      ...
      { type: 'stack', direction: 'row', children: [
        { type: 'text', text: `Total ↓${fmtBytes(d.netRx)}`, font: { size: 10, family: 'Menlo' }, textColor: C.net },
        { type: 'spacer' },
        { type: 'text', text: `Total ↑${fmtBytes(d.netTx)}`, font: { size: 10, family: 'Menlo' }, textColor: C.swap },
      ]},
      divider,

      // Last Command 行：字体放大到14号+彩色高亮
      { type: 'stack', direction: 'row', alignItems: 'center', gap: 4, children: [
        { type: 'image', src: 'sf-symbol:terminal', color: C.muted, width: 13, height: 13 },
        { type: 'text', text: 'Last Command', font: { size: 'caption1', weight: 'bold' }, textColor: C.text },
        { type: 'spacer' },
        { type: 'text', text: d.lastCmd, font: { size: 14, family: 'Menlo', weight: 'medium' }, textColor: C.cmd, maxLines: 2, minScale: 0.6 },
      ]},
      divider,
